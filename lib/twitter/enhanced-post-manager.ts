  ...
  try {
    ...
    if (draft.thread_structure && typeof draft.thread_structure === 'object') {
      ...
      if (threadData.type === 'thread' && Array.isArray(threadData.posts)) {
        ...
        // 最初の投稿にハッシュタグを追加（まだ付いていない場合）
        if (tweets.length > 0 && draft.hashtags && draft.hashtags.length > 0) {
          const hashtagsStr = draft.hashtags.map((tag: string) => 
            tag.startsWith('#') ? tag : `#${tag}`
          ).join(' ')
          
          // 最初の投稿にハッシュタグがまだ含まれていない場合のみ追加
          if (!tweets[0].includes('#')) {
            tweets[0] = `${tweets[0]}\n\n${hashtagsStr}`
          }
        }
      } else {
      ...
      // 単一投稿にもハッシュタグを追加（まだ付いていない場合）
      if (draft.hashtags && draft.hashtags.length > 0 && !tweets[0].includes('#')) {
        const hashtagsStr = draft.hashtags.map((tag: string) => 
          tag.startsWith('#') ? tag : `#${tag}`
        ).join(' ')
        tweets[0] = `${tweets[0]}\n\n${hashtagsStr}`
