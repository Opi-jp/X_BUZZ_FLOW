...
### 背景
- **キャリア**: NAKEDという映像制作会社を共同創業（25歳時）。23年にわたりクリエイティブディレクター/プロデューサーとして活動
- **実績**: 2012年よりプロジェクションマッピングで有名に。映像/デザイン/WEB/マーケティング分野で実績多数
- **現状**: 50歳を超えてセカンドキャリアを模索。LLMによる働き方変革に注目
- **目標**: 自由な時間を確保しつつ、X（Twitter）での影響力とベーシックインカムを確立

### 発信軸
1. **メインテーマ**: クリエイティブの発想でLLMをどう活用し、新しい時代を生き抜くか
   - 従来の「LLMをクリエイティブ現場に活かす」とは逆の視点
   - 23年のクリエイティブ経験を活かした独自の視点

2. **発信内容の種類**:
   - バズツイートへのRP（引用RT）コメントでエンゲージメント獲得
   - AI関連ニュースの厳選共有
   - 独自の知見・解釈・未来予測を加えた差別化コンテンツ
   - 働き方の未来予測
...
4. **バズツイート収集機能の修正**
   - KAITO APIの収集処理でDB保存されない問題を解決
   - 構文エラー（インデント不整合）を修正
   - エラーハンドリングとログ出力を改善
   - 重複チェックが正常に動作することを確認
   - 20件のバズツイートをDBに正常保存

5. **検索クエリの最適化（バズ戦略に基づく）**
   - ライアン氏のバズ戦略ガイドラインをCLAUDE.mdに記載
   - 収集プリセットを7個作成：
     - AI・生成AIトレンド（min_faves:300）
     - AIツール活用術（min_faves:500）
     - AI×クリエイティブ（min_faves:300）
     - 働き方の未来（min_faves:500）
     - 異常値系バズ投稿（min_faves:1000）
     - 自分語り誘発投稿（min_faves:1000）
     - X運用ノウハウ（min_faves:500）
   - 検索クエリの最適化：
     - `min_faves:`パラメータの重複チェック
     - デフォルトエンゲージメント率0.5%設定
     - バズレベルタグ付け（mega_buzz, high_buzz, buzz）
     - 異常値系・トレンドワード自動判定
   - 収集結果の分析機能追加：
     - 平均エンゲージメント統計
     - バズレベル別カウント
   ...
   - 検索精度向上（v2）：
     - プロフィール検索除外オプション追加
     - コンテンツ関連性チェック機能
     - 働き方関連：エンタメ系除外、必須キーワードチェック
     - AI関連：実質的内容チェック、URLのみ投稿除外
     - より複雑な検索クエリ構造（AND/OR条件、除外キーワード）

## Xバズ戦略ガイドライン（ライアン氏のnoteより）

### 第1章：土台作り
1. **発信ジャンル/方針の明確化**
   - ジャンルを絞ることでフォロワー層を統一
   - 複数ジャンルの共通項を狙う（ヴェン図の赤丸部分）

2. **平均インプ500超えを目指す**
   - アルゴリズム：既存フォロワー→高評価→非フォロワーへ拡散
   - 500未満の対策：
     - 大手アカへの引用ポスト
     - バズ投稿へのリプライ/引用
     - コミュニティ所属
...
### 第2章：ネタ探し
- **X検索コマンド**: `キーワード min_faves:300`
- 5W1Hによる連想でアイデア出し
- インプ上限の予測（ターゲット層の広さで判断）

### 第3章：ポスト作成
1. **1行目が最重要**
   - 異常値系：具体的数値で平均からかけ離れた内容
   - トレンドワード：生成AI、ChatGPT、MBTI等
   - 常識への逆張り/コンプレックス刺激

2. **投稿の型**
   - ひとくちトリビア系：学びのある体験談
   - 自分語り誘発系：学歴、職歴、恋愛、資産等
...
- サブアカ活用（本アカの質向上）
- 同ジャンルへの積極的いいね
- 過度な動きは禁止（ミュート/ブロック回避）

### 第5章：バズ事例分析
**「20代で億稼げる」投稿（21万インプ）の成功要因：**
- 既知の型の活用
- 反応の予測（賛否両論）
- 型の鮮度（最近使われていない）
- フォロワーとの関連性
