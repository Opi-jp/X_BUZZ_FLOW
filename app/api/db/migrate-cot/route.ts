...
export async function POST() {
  ...
  try {
    ...
    // 外部キー制約は既に存在する場合エラーになるので、個別に処理
    for (const query of fkQueries) {
      try {
        await pool.query(query);
      } catch (error) {
        console.log('FK constraint may already exist:', error.message);
      }
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'CoT tables created successfully',
      tables: result.rows,
      details: {
        enums: ['cot_session_status', 'cot_phase_step', 'cot_phase_status', 'cot_draft_status'],
        tables: ['cot_sessions', 'cot_phases', 'cot_drafts', 'cot_draft_performance'],
        indexes: indexQueries.length,
        foreignKeys: fkQueries.length
      }
    });
  } catch (error) {
