...
専門分野「${config.config?.expertise || 'AI × 働き方'}」で、${config.config?.platform || 'Twitter'}向けのバズるコンテンツを高速生成してください。`
        },
        {
          role: 'user',
          content: `現在の${config.config?.expertise || 'AI × 働き方'}に関するトレンドから、48時間以内にバズる可能性が最も高い投稿を1つ生成してください。
...
以下のJSON形式で回答：
{
  ...
  "viral_score": 0.8,
  "content": {
    "text": "${config.config?.platform || 'Twitter'}用の完全な投稿テキスト（改行・絵文字含む）",
