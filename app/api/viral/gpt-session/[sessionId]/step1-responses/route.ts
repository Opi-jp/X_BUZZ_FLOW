...
【あなたの視点】
- 専門分野: ${config.expertise}
- 発信プラットフォーム: ${config.platform}
- コンテンツスタイル: ${config.style}

この視点から、以下のすべてのカテゴリで起きている出来事を分析し、
「${config.expertise}」の専門家として独自の解釈や関連付けができるバイラル機会を探してください。

例：専門分野が「AI × 働き方」の場合
- 有名人の発言 → AIが仕事に与える影響についてのコメント
- 企業ニュース → AIツール導入による生産性向上の事例
- 文化的現象 → AIアートの流行と創造性の議論
...
ウェブ検索を使用して、以下の8カテゴリから最新のニュースやトレンドを調査してください：
